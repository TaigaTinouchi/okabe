...
サーバーが起動していれば、毎朝 7:30（`MORNING_SUMMARY_CRON` で変更可、Asia/Tokyo）に
当日の予定サマリーが通知としてアプリに届く。アプリが閉じていても受信箱に残り、
次回起動時の catch-up で必ず届く（ADR-0003 の原則そのまま）。

- サマリーの整形は軽量モデル（`ANTHROPIC_MODEL_LIGHT`、既定 claude-haiku-4-5）に振っている。
  `LlmProvider` の tier フックの最初の適用（階層ルーティングの入口）
- 通知は会話タイムラインの一部なので、届いた通知に対してそのまま
  「1件目の詳細教えて」と続けられる
- ジョブ実行履歴は `job_runs` テーブルに残る（多重実行防止・リトライ最大3回。ADR-0007）
- ジョブの無効化: `DISABLED_JOBS=morning-summary`
